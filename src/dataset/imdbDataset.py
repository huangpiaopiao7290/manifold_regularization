...
# Training function
def train(model, train_loader, valid_loader, epochs, device, writer):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch_label in train_loader:
            ...
            optimizer.zero_grad()
            loss = model(batch_idx, batch_label)
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    train_texts, train_labels = read_data(file_paths['train'], 100)
    valid_texts, valid_labels = read_data(file_paths['valid'], 100)
    test_texts, test_labels = read_data(file_paths['test'], 100)
    ...
    model_ = TextCNN(wordsEmbedding, num_classes=num_classes, max_len=MAX_LEN).to(device)
    ...
    train(model_, trainLoader, validLoader, epochs_, device, writer)

    model_.eval()
    correct = 0
    total = 0
    with torch.no_grad():
        for batch_idx, batch_label in testLoader:
            batch_idx = batch_idx.to(device)
            batch_label = batch_label.to(device)
