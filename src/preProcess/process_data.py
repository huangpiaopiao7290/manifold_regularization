...
def create_cifar10_dir(file_path):
    ...
    # 训练集(无标签)
    if not os.path.exists(os.path.join(processed_dir, file_path, " train", "unlabeled")):
        os.makedirs(os.path.join(processed_dir, file_path, "train", "unlabeled"))
        logging.log(logging.INFO, "train/unlabeled created...")
...
def decode_and_generate(batch_file, target_base_file):
    ...
    """
    # 获取训练集路径
    train_list: list = find_filenames_with_keywords(batch_file, "data_batch")
    # 获取测试集路径
    test_list: list = find_filenames_with_keywords(batch_file, "test_batch")
...
def split_ssl_dataset(base_dir, num_label=5000, num_unlabel=45000):
    ...
    if not os.path.exists(unlabeled_dir):
        raise FileNotFoundError(f"{unlabeled_dir} not found")
    ...
    for label in labels:
        ...
        # 移动图像数据
        # move_images(label_dir, unlabeled_dir, num2move / images_num)
        tasks.append((label_dir_path, unlabeled_dir, num2move / images_num))
